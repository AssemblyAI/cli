...
def test_list_voices_json_emits_machine_readable_array(monkeypatch):
    monkeypatch.setattr(
        "aai_cli.commands.agent._exec.run_session",
...
def test_agent_sample_uses_hosted_clip(monkeypatch):
    ...
    monkeypatch.setattr("aai_cli.commands.agent._exec.FileSource", fake_file_source)
    seen = _capture_run_session(monkeypatch)
...
def test_agent_mic_shows_start_talking_notice(monkeypatch):
    ...
    monkeypatch.setattr("aai_cli.commands.agent._exec.DuplexAudio", FakeDuplex)
...
def test_agent_show_code_prints_without_session(monkeypatch):
    ...
    called = []
    monkeypatch.setattr(
        "aai_cli.commands.agent._exec.run_session", lambda *a, **k: called.append(True)
    )
...
def test_agent_show_code_file_source_warns_on_stderr(monkeypatch):
    ...
    monkeypatch.setattr("aai_cli.commands.agent._exec.run_session", _boom)
