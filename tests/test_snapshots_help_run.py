"""Golden snapshots pinning the exact `--help` render of the run commands.
...
file. A diff means the visible CLI changed; if intentional, refresh with::
    ...
    uv run pytest tests/test_snapshots_help_run.py --snapshot-update
...
@pytest.mark.parametrize("argv", help_group_argvs("run"), ids=lambda a: "_".join(a))
