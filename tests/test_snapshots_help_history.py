"""Golden snapshots pinning the exact `--help` render of the history commands.
...
file. A diff means the visible CLI changed; if intentional, refresh with::
    ...
    uv run pytest tests/test_snapshots_help_history.py --snapshot-update
...
@pytest.mark.parametrize("argv", help_group_argvs("history"), ids=lambda a: "_".join(a))
