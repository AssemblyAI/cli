...
def test_todo_collector_ignores_a_non_write_todos_tool_carrying_todos_args():
    # Only write_todos populates the plan: the `name == write_todos AND args-is-a-dict` guard must
    # reject a *different* tool whose args happen to carry a `todos` key (an `or` there would let a
    # sibling tool's payload masquerade as the plan).
    collector = plan.TodoCollector()
    collector.note_chunk(
        AIMessage(
            content="",
...
def test_todo_collector_ignores_fragments_from_a_different_tool_index():
    # A write_todos call opens at index 0; a sibling tool's arg fragment arrives at index 1. The
    # `index == open-index` guard must drop it — folding a foreign fragment into the buffer (an `or`
    # there) corrupts the todos JSON. The fakes expose only `tool_call_chunks` (no `.tool_calls`),
    # so the buffer is the *sole* source: under the `or` mutant the buffer no longer parses and
    # take() returns None instead of the plan, which this assert catches.
    import types

    def chunk(fragments: list[dict[str, object]]) -> types.SimpleNamespace:
        return types.SimpleNamespace(tool_call_chunks=fragments)
    ...
    # A different tool's fragment at index 1 — must NOT be appended to the write_todos buffer.
    collector.note_chunk(chunk([{"name": None, "args": "GARBAGE", "index": 1}]))
    # Close the write_todos JSON at its own index 0.
    collector.note_chunk(chunk([{"name": None, "args": "]}", "index": 0}]))
    assert collector.take() == plan.TodoUpdate((plan.TodoItem(content="A", status="pending"),))
