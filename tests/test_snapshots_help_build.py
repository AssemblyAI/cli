"""Golden snapshots pinning the exact `--help` render of the build commands.
...
file. A diff means the visible CLI changed; if intentional, refresh with::
    ...
    uv run pytest tests/test_snapshots_help_build.py --snapshot-update
...
@pytest.mark.parametrize("argv", help_group_argvs("build"), ids=lambda a: "_".join(a))
