"""Write-approval (`--files`) tests for the `assembly live` reply brain.

Split out of `test_agent_cascade_brain.py` to keep both files under the 500-line gate.
These drive the *real* deepagents graph (gated on write_file/edit_file) against a fake
chat model — pytest-socket stays armed; no sockets — and assert the approver is consulted
and bracketed by ApprovalPause events.
"""
