...
def test_stream_mic_renders_turns(monkeypatch):
    ...
    monkeypatch.setattr("aai_cli.commands.stream._exec.client.stream_audio", _drive_turns)
...
def test_stream_broken_pipe_exits_zero(monkeypatch):
    ...
    monkeypatch.setattr("aai_cli.commands.stream._exec.client.stream_audio", raise_broken_pipe)
    result = runner.invoke(app, ["stream"])
    assert result.exit_code == 0
...
def test_stream_youtube_url_downloads_then_streams(monkeypatch, tmp_path):
    ...
    with wave.open(str(fake), "wb") as w:
        ...
        w.setframerate(16000)
        w.writeframes(b"\x00\x01" * 100)
    monkeypatch.setattr("aai_cli.commands.stream._exec.youtube.download_media", lambda url, d: fake)
    seen = {}
    ...
    monkeypatch.setattr("aai_cli.commands.stream._exec.client.stream_audio", fake_stream)
