...
def _sidecar_record(
    ...
    *,
    ...
    return {
        ...
        "audio": paths.audio.name if plan.save_audio else None,
...
def write_outputs(
    ...
    *,
    ...
    ``title`` is the ``--auto-name`` headline (None when not requested); when it slugs
    to a non-empty stem the provisional transcript/WAV are renamed to carry it. ``note``
    is the final ``--llm`` answer, written as ``<stem>.md`` when present. The sidecar is
    always written. Returns the final paths.
    """
    ...
    if final.stem != provisional.stem:
        ...
        if plan.save_audio and provisional.audio.exists():
            _rename(provisional.audio, final.audio)
    if note is not None:
